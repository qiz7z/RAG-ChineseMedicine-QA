...
#### 指标介绍（读下面这张表之前，先看清这三个词）

> - **Hit@5：回答「有没有找着」。** 逐题只有 0 或 1，记 1 的条件是**前 5 名里至少出现一个期望药**；
>   公式 = 命中的题数 / 可评测题数（`@3`、`@1` 就是换成前三名、第一名，K 越小越严）。
>   注意有 **loose / strict 两档**：loose 是药名双向子串（`人参健脾丸` 也算命中「人参」）、**不判章节**；
>   strict 要求药名精确（含该药饮片条目 `人参-饮片`）**且**期望章节在切片正文里真实出现。
> - **Recall@5：回答「找着了几个」。** 逐题是 0~1 的分数，**不判章节**；
>   公式 = |top-5 命中的期望药| / |期望药|，再对全部题取平均。
>   多药题（如「哪些药材补气」期望 4 个药）只答对 1 个就只拿 0.25——**这正是 Hit@5 会虚高的地方**。
>   另附 `全召回@5` = 期望药**全部**进 top-5 的题占比（最严读法）。
> - **幻觉率：回答「有没有瞎说」。** 它是**论断级**指标，与检索是否命中无关（检索成功也可能夹带来源不支持的话）；
>   公式 =（**无据论断 + 与来源矛盾的论断**）/ 被判定的**正文论断**数
>   （另有标题行与表格行未参与判分，属覆盖面缺口，见 [docs/13](docs/13_核心章节缺失与判分假阳性修复.md)）。
