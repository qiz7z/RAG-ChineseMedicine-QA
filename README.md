...
**检索**（**关重排**为默认配置；全量 109 题计分，两档分母一致）

| 引擎 | loose@5 | strict@1 | strict@3 | strict@5 | strict MRR |
|------|---------|----------|----------|----------|-----------|
| 手撕版 `src/` | 96.33% | **75.23%** | 85.32% | **91.74%** | **0.8118** |
| LangChain `langchain_app/` | 96.33% | **72.48%** | 84.40% | **88.99%** | **0.7878** |
| （消融）开重排 | 97.25% | 68.81% | — | **93.58%** | 0.7891 |

> 开重排**提召回、降首选精度**（@5 +1.84 / +4.59pp，@1 −6.42 / −3.67pp）——是**取舍**而非"负收益"；
> 早先"一律负收益"的结论是未修字的语料造成的，详见 [docs/07](docs/07_LangChain标准版架构.md)。
> 基线 100 题口径、分题型明细见下方「检索指标明细」「分类型结果」。

**生成**（109 题，agnes-2.5-flash；**分母 = 参与判分的正文论断**）

| 指标 | 值 | 口径 |
|------|----|------|
| **幻觉率** | **9.21%** | (无据 32 + 矛盾 12) / **478** 条正文论断 |
| 论断有据率 | **90.79%** | 434 / 478 |
| 回答有据率 | **76.47%** | 要求**全部**论断有据，对"每题论断数"敏感 |
| 关键词覆盖率 | **82.58%** | ⚠️ 只查关键词是否出现，**不是准确率** |
