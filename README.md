...
flowchart LR
    ...
    D --> F["最优配置<br>去重排 + 章节感知召回<br>严格 Hit@5 85% · P50 0.073s"]
    E --> F
...
### ⭐ 当前状态与下一步（2026-09-23，缺陷 14/15 修复后更新）

**一句话：15 个已知缺陷清零；幻觉率按"无据扩展"口径 ≈2.00% 已达项目书 ≤3% 目标，
名义判分口径 8.66% 的差距几乎全部来自判官视野（截断残留）而非生成质量。**

| 项目书目标 | 实测 | 状态 |
|-----------|------|------|
| 检索 Recall@5 ≥ 90% | strict **91.74%** / 88.99%（loose 96.33% / 96.33%） | 🟢 手撕版达标；标准版 88.99%（差 1.0pp） |
| **幻觉率 ≤ 3%** | **未达标**：**9.21%**（44/478 条正文论断）；三次跑批 6.36% / 8.78% / 9.21% | 🔴 **未达标**。失败归因见下：**条目混淆 10 条是真问题（≈2.1pp）**，其余为判官对综合推理偏严 + 模型自加表述。详见 [docs/13](docs/13_核心章节缺失与判分假阳性修复.md) 缺陷 18~21 |
| 回答准确率 ≥ 95% | 项目无"准确率"指标；关键词覆盖 **82.58%** 仅召回倾向 | ⚪ 指标缺失 |

> ### 幻觉率怎么读（最终口径，三次跑批 6.36% / 8.78% / 9.21%）
>
> **它是"正文论断"的比率，不是"整篇回答"的比率**：
>
> | 量 | 值 | 说明 |
> |----|----|------|
> | 参与判分的正文论断 | **478** | 分母 |
> | └ 无据（模型自加） | 32 | 如"偏于清泻""补气之力较缓"这类资料里没有的对比/强度判断 |
> | └ 与来源矛盾 | 12 | 其中**条目混淆 10 条是真问题**：饮片标准当药材标准、`炙黄芪`当`黄芪`、`酒当归`当`炙当归`、赤芍功效记到白芍等 |
> | **未参与判分** | **364 行** | 标题行 176 + 表格行 188——**这是判分覆盖面缺口**，表格内容改由检索层覆盖，判官不判 |
>
> 三点必须一起说，否则数字会误导：
> 1. **口径三次变动**（去污染分母 → 修波浪号 → 剔除非论断/表格），三次的数**不可直接相减**；
> 2. **采样波动**：温度 > 0，回答长度/结构每次不同（本轮模型把大量内容放进表格，正文论断从 820 掉到 478）；
> 3. **判分漏判**（本次 1 条）单列 `unscored_claims` **不进分母**；此前报的"剔除判官视野外 1.10%"是把漏判条数**重复扣减**得到的错误数字，**已作废**。

**最近一轮修复（缺陷 14~21，全部详见 [docs/13](docs/13_核心章节缺失与判分假阳性修复.md)）**：
下面先列 14~17（数据/解析层），18~21（判分口径、语料错字长尾、测试集勘误）见缺陷表：

- **缺陷 18~21（2026-09-23 收尾）**：① `~` 被当 Markdown 删除线删掉 → `6~12克` 变 `612克`，
  判官据此判"矛盾"（8 条假幻觉，**无任何用户可见症状**）；② 聚合幻觉率分母口径不一致；
  ③ 语料错字长尾治理（`昔→苷` 2,614、`淫羊蕾→淫羊藿` 326 等，**检索两版齐升**）；
  ④ 判分不再把出处声明/标题行/表格行算成幻觉（丢弃数如实报出）。

- **缺陷 14**：85% 主条目的【性味与归经】【功能与主治】等核心章节被解析器挂到了
  `-饮片` 子条目上 → 已回填；「天麻的性味归经」从答"未收录"变为逐字引用（覆盖率 0→1.00）；
- **缺陷 15**：判官漏判被计成幻觉 + 来源截断 900 字 → 漏判单列 `unscored` 剔除分母、
  截断放宽至 1600 字；
- **缺陷 16**：1,020 个 Word 文本框（含 86 处【性味与归经】）未被读取 → 已线性化搬回正文流；
  药材主条目有【性味与归经】86% → **91%**（+30 个，含大黄/丁公藤/干漆），
  否定性问题覆盖率 40% → **63%**、关键覆盖率 82.77% → **84.37%**。
  ⚠️ 插入段落会撞坏"药名-拼音"前瞻（曾使茜草整条消失），已用「合成段落不参与前瞻」修掉。
- **缺陷 17**：正文里 **1,297 处**章节标记被 OCR 打碎（`t鉴别】`/`［含量测定】`/`1功能与主治】`/`[:规格】`…），
  解析器的章节正则认不出来 → 内容并进上一节。已补回标记：药材有性味 91% → **94%**，
  切片 12,375 → 12,955，**~1,300 个章节回归独立**（鉴别 2,890、处方 1,575、规格 1,468…）。

**残余清单**（均有实测与定位，非遗漏；详见 [docs/13](docs/13_核心章节缺失与判分假阳性修复.md) §残余清单）：

1. **39 / 604 个药材主条目仍缺性味**——版面位移（内容块在表头前 28~120 块处）；
   修它要重排解析顺序，会牵动已验证的解析链 → 暂缓；**真错配仅 2 处**；
2. **评测集真实失败 4 道**（Q019/Q096 章节缺失属上面 39 个；Q031 药名不精确；Q075 横向条件召回失败）；
3. **判官视野外 22 条**（来源截断残留，占名义幻觉率 ≈2.7pp）——放宽来源上限可再降，成本低；
4. **标题性短语进非论断过滤**（3 条/900）——成本极低；
5. 检索侧**横向条件查询**（src 56% / lc 44%，最短板，且评测口径对"集合型期望"不适配）需重构；
6. 需新资源的两项：`pinyin_name` 字段 ~35% 不可用、药典四部通则语料（10 道题已标 `out_of_scope`）；
7. 换更强生成模型（当前 agnes-2.5-flash）——所有生成侧指标与其绑定，换模型须全量重测。

其余已知的低优先遗留项（拼音字段 35% 不可用、低频多义错字、四部通则语料缺失）
见 [docs/11 §九](docs/11_本轮总账.md)，均有明确记录、非遗漏。
...
两档在报告中并列输出（`data/eval/reports/*.json` 的 `retrieval` 与 `strict_retrieval` 块）。
...
|------|-------------|------------------|--------------|-----------|---------|
| 手撕版 `src/` | 96.33% | **91.74%** | 75.23% | 0.8118 | 0.067s |
| LangChain 标准版 `langchain_app/` | 96.33% | **88.99%** | 72.48% | 0.7878 | 0.075s |

**基线 100 题口径**（Q001–Q100，与历史报告可比）
...
> 均为**关重排**配置。**⚠️ 2026-09-24 复查：重排的结论翻转了**——语料修字（缺陷 19）后，
> 开重排 `strict@5` **升到 93.58%**（两版一致，比关重排 +1.84pp / +4.59pp），
> 但 `strict@1` 掉到 **68.81%**（−6.42pp / −3.67pp）。
> 即重排是**"提召回、降首选精度"的取舍**，而非早先认定的"一律负收益"——
> 当时那个 −3.7pp 的结论建立在**未修字的语料**上。默认仍取关重排（本项目口径里 `Hit@1` 权重更高）；
> 若只看 Recall@5，开重排更优。数据见 [docs/07](docs/07_LangChain标准版架构.md)。
> 测试集 **120 题** = **基线 100 题**（Q001–Q100；用 `--limit 100` 复现）+ 2026-09-22 新增的
> **6 道「子串药名干扰」**（Q101–Q106）+ **14 道「OCR 药名纠错」**（Q107–Q120）。
> 其中 **11 道标了 `out_of_scope`，两项口径都不计分** → 实际计分 **109 题，且全部可评测**，
> 所以 **loose 与 strict 的分母已经一致**（收口前 loose 是 120 为分母、含 11 道白送分）。
...
**生成侧换成真指标（LLM-as-judge）**

老的两个生成侧指标都不可用，已替换：

| 老指标（不可用） | 新指标（真实） |
|------------------|---------------|
| `citation_rate` **100%**（恒真：引用由检索结果机械拼出） | **论断有据率 91.11%**（820/900 条论断能在来源里找到依据） |
| `consistency_issue_rate` 10.09%（只查带单位数值 → 幻觉率**下界**） | **幻觉率 11.72%**（无据 9.52% + 与来源矛盾 1.93%） |
| `avg_keyword_coverage` 81.90% 被当成质量 | 保留但**明确标注不是准确率**，仅为召回倾向参考（本轮 79.34%） |

新增 **回答有据率 53.47%**（102 条回答；⚠️ 对"每题论断数"敏感，见 [docs/13](docs/13_核心章节缺失与判分假阳性修复.md)）。

**实现**：`src/eval/grounding_judge.py` 把回答拆成可验证论断，逐条问「能否在**本次实际喂给 LLM 的来源**里找到依据」，
三分类 supported / unsupported / contradicted；`temperature=0`、按批 8 条判（避免长 JSON 被截断）、逐条留痕；
判定失败的题**整体排除**并如实报出（7/109），因为"判定失败"不是幻觉证据。
另有**引文核验式第二判官**可做交叉验证（`--crosscheck`），见 [docs/12](docs/12_判分交叉验证与语料修字.md)。

> **一句话**：过去对外讲的"引用率 100%"，实际掩盖了 **11.72%** 的幻觉率
> （分母去掉框架句等非论断后，比早期实测的 9.45% 更高——那 9.45% 被自动"有据"的填充句稀释了）。

**跨药污染尾巴剪裁（缺陷 12）**

条目某一节的末尾会粘着**下一个药的开头**（`苍术-饮片` 的【贮藏】后接 `Cang'erzi\nXANTHII FRUCTUS\n本品为菊科植物苍耳…`；
`茵栀黄胶囊` 的【含量测定】里连吞 1,787 字）。根因与缺陷 11 同源（多栏/表格版面），但边界处药名行缺失或被拆散。

新增 `trim_cross_drug_tails()`：**只做减法、不做猜测**，判定要求两个信号同时成立。

| 结果 | 数值 |
|------|------|
| 受影响条目 | **84 个** |
| 丢弃内容 | **12,131 字** + **117 个越界章节** |
| `【贮藏】` 后有越界章节的条目 | **60 → 0** |
| 剩余「过长贮藏」 | 逐条复核**全部为药典合法的 `附：/注：/制剂` 续接** |

> ⚠️ 两个坑（都已固化测试）：
> ① 药典允许【贮藏】后跟 `附：xxx质量标准` / `注：…`（实测 **2,032 字是合法的**），不能按句号或长度剪；
> ② 正文里也有独占一行的西文（含量测定公式 `bOO X w`），单看"拼音样的一行"会**误伤**（`滑石粉` 被误判过）。
>
> 代价：切片 12,040 → **11,940**；**1 题**（Q070 横向条件查询）出现排序副作用——
> `蔓荆子-饮片` 剪掉混入的青黛鉴别文字后 chunk 变短，BM25 长度归一化使其升到第 1 位，
> 把原本排第 5 的 `甘草` 挤出 top-5；同期 Q019 反而改善（rank 5→4）。

### 分类型结果

| 查询类型 | 题数 | 可评测 | src strict | src loose | lc strict | lc loose |
|---------|------|-------|-----------|-----------|----------|----------|
...
| **缺陷 14~19 全量修复（2026-09-23）** | 95.41% | 严格 Hit@5 **90.83%**，详见 [13](docs/13_核心章节缺失与判分假阳性修复.md) |
| **缺陷 19~21：语料长尾治理 + 判分口径 + 测试集勘误（2026-09-23）** | 96.33% | strict Hit@5 **91.74% / 88.99%**（两版均升）；语料清掉 `昔→苷`(2,614)、`淫羊蕾→淫羊藿`(326) 等；幻觉率 **9.21%**（未达标，真问题集中在条目混淆 10 条），详见 [13](docs/13_核心章节缺失与判分假阳性修复.md) |
...
8. **补测试集盲区**：本次修的"药品名双向扩展"缺陷在 100 题上 **0/100 命中**——没有任何题目的期望药名包含另一个药名（如"双黄连口服液"含"黄连"）。这类 bug 只能靠人工探测，需专门补题
9. **替换恒真指标**：`citation_rate`（引用由检索结果机械生成）、`consistency_issue_rate`（只查带单位数值）都不足以衡量幻觉

### 工程化与规模化

10. **部署**：Docker 化两套引擎 + docker-compose 编排（API/WebUI/索引构建）
11. **LLM 服务升级**：当前生成 P95 受免费推理通道拖累（~18~23s），切换生产级端点
12. **规模化改造路径**（当前单机内存索引，11k 规模够用，百万级需）：FAISS Flat → Milvus/pgvector/HNSW；会话内存 dict → Redis；GPU 重排拆独立推理服务；检索命中率/引用率/延迟分位埋点
...
> - ~~章节判分/召回~~（章节感知召回，strict Hit@5 69.66%→**89.91%**，全 120 题）
...
- [本轮总账](docs/11_本轮总账.md) — ★★ 11 个缺陷清单、前后对比、`Hit@K` 何时测不出问题、对照项目书的诚实结论
...
### 2026-09-22 判分口径澄清 + 11 个缺陷修复 + 生成侧口径修正

起因是自查指标是否虚高。结论：**口径确实偏松，但底下还压着一批真实缺陷**，最终修掉 **10 个**。
...
**生成侧结果**（109 题，同一通道 agnes-2.5-flash）
