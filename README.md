...
|-----------|-------------------|------|
| 检索 Recall@5 ≥ 90% | strict **91.74%**（src）/ **88.99%**（lc）；loose 96.33% / 96.33% | 🟢 手撕版达标；标准版差 1.0pp |
...
| 引擎 | 配置 | loose@1 | loose@3 | loose@5 | strict@1 | strict@3 | strict@5 | strict MRR | P50 延迟 |
|------|------|---------|---------|---------|----------|----------|----------|-----------|---------|
| 手撕版 `src/` | **关重排（默认）** | 91.74% | 93.58% | 96.33% | **75.23%** | 85.32% | **91.74%** | **0.8118** | 0.056s |
| 手撕版 `src/` | 开重排（消融） | 92.66% | 96.33% | 97.25% | 68.81% | 88.07% | **93.58%** | 0.7891 | 0.221s |
| LangChain `langchain_app/` | **关重排（默认）** | 91.74% | 94.50% | 96.33% | **72.48%** | 84.40% | **88.99%** | **0.7878** | — |
| LangChain `langchain_app/` | 开重排（消融） | 93.58% | 96.33% | 97.25% | 68.81% | 85.32% | **93.58%** | 0.7859 | — |

> **报告（均已在 `data/eval/reports/`）**：src `eval_retrieval_20260923_130214`（关）/ `…20260924_003935`（开）；
> lc `eval_lc-std-noRerank_retrieval_20260923_130234`（关）/ `eval_lc-std_retrieval_20260924_004029`（开）。
> 后两份是本轮之后生成的，文件名已带 `noRerank`/`rerank` 后缀（旧报告无后缀）。
>
> **重排是取舍**：`strict@5` +1.84 / +4.59pp，但 `strict@1` −6.42 / −3.67pp、延迟 ×4（0.056s→0.221s）。
> 早先"一律负收益"的结论是未修字的语料造成的，详见 [docs/07](docs/07_LangChain标准版架构.md)。
> 基线 100 题口径见下方「检索指标明细」；**题型分层见「分类型结果」**；历史阶段见「优化历程」。
...
#### 本轮修了什么（缺陷 14~21，详见 [docs/13](docs/13_核心章节缺失与判分假阳性修复.md)）
...
4. **横向条件查询**（src 56% / lc 44%）是最短板，且评测口径对"集合型期望"本身不适配，需重构；
...
| 查询类型 | 题数 | 可评测 | src strict@5 | src loose@5 | lc strict@5 | lc loose@5 |
|---------|------|-------|-------------|-------------|------------|------------|
| 单药品单属性查询 | 25 | 25 | 92% | 100% | 92% | 100% |
| 单药品多属性查询 | 15 | 15 | 93% | 100% | 87% | 100% |
| 精确数值查询 | 15 | 15 | **100%** | 100% | 93% | 100% |
| 跨品种比较查询 | 10 | 10 | **100%** | 100% | **100%** | 100% |
| 多轮对话 | 10 | 10 | 90% | 100% | 90% | 100% |
| 否定性问题 | 5 | 5 | **100%** | 100% | 80% | 100% |
| **横向条件查询** | 9 | 9 | 44% | 56% | 44% | 56% |
| 子串药名干扰 | 6 | 6 | **100%** | 100% | **100%** | 100% |
| OCR 药名纠错 | 14 | 14 | **100%** | 100% | **100%** | 100% |

> 报告：同表 1（src `eval_retrieval_20260923_130214`、lc `eval_lc-std-noRerank_retrieval_20260923_130234` 的 `by_type` 段）。
> 上一版此表是**旧口径**（如 src 单属性 88%），已按最新报告刷新。
