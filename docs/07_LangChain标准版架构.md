...
### 复查（2026-09-24，语料修字后重跑）：结论**翻转**

> ⚠️ **这一节推翻了 09-23 的"负收益"结论。** 当时语料还带着系统性错字（`昔→苷` 2,614 处等）、
> 药名排序键也不完整，重排把「药名贴合度」排序整个推翻，于是 `strict@1` 崩到 53.21%。
> 缺陷 19（语料错字治理）落地后，在**全量 109 题**上重跑开关对照：

| 引擎 | 配置 | loose@5 | strict@5 | strict@1 | strict MRR |
|------|------|---------|----------|----------|-----------|
| src | 关重排（默认） | 96.33% | **91.74%** | **75.23%** | **0.8118** |
| src | 开重排 | **97.25%** | **93.58%**（+1.84pp） | 68.81%（−6.42pp） | 0.7891 |
| lc | 关重排（默认） | 96.33% | **88.99%** | **72.48%** | **0.7878** |
| lc | 开重排 | **97.25%** | **93.58%**（+4.59pp） | 68.81%（−3.67pp） | 0.7859 |

**新结论**：重排不是"一律负收益"，而是**提升召回、牺牲首选精度**——
`strict@5` 最高 +4.59pp，但 `strict@1` 掉 3.7~6.4pp。默认配置仍取**关重排**，
因为本项目口径里 `Hit@1`（首选是否就是正主）权重更高；
**若目标只看 Recall@5 ≥ 90%，开重排更优**（两版均 93.58%）。

（GPU 上重排约 +0.17s/题；CPU 会再慢一个数量级。）

#### （历史记录）09-23 为何会跌到 53.21%

**两版 strict@1 当时都恰好跌到 53.21%**——不是巧合，是同一个机制性原因：

1. **重排覆盖了缺陷 9 的排序键**。src 管线顺序是 `_promote_section_hits`（含药名贴合度）
   **先跑**、`reranker.rerank()` **后跑**——重排按交叉编码分数**重新排序**，
   把「`黄芪-饮片` 应排在含黄芪成药之前」这条 strict 口径对齐的原则整个推翻；
2. **交叉编码器偏爱"复述问题的成药散文"**。丢失的 5 题全是同一模式：
   `黄芪的用法用量` → 关重排 top1 = `黄芪-饮片/完整条目`，
   开重排 top1 = `黄芪颗粒/临床应用`——成药的临床应用小节用大白话复述了
   "用法用量"，与问题字面更近，交叉编码分数更高；
3. **`rerank(top_k=5)` 丢弃 top-5 之外的一切** → strict@5 也掉了 3.7pp（召回被截断）。

**结论维持并加强**：在本语料上不要开重排。若未来想再试，方向不是"直接换上"，
而是**把药名贴合度融进重排分数**（如 `final = rerank_score + α·exactness`），
让重排只在"同药候选"内部精排——但当前数据不支持为 +0.17s/题的代价做这件事。
...
> |---|---|---|---|---|
> | 手撕版（同条件对照） | 58.6% | 92% | 12% | 4.61s |
> | 标准版完整（含重排） | 60.5% / 55.3%² | 100% / 98% | **13%** | 5.44s / 11.93s² |
> | 标准版去重排 | 59.2% | 99% | 未测 | 5.92s |
>
> ² 免费通道两轮运行，第二轮通道明显变慢（P95 57.6s，1 题超时按 0 分计）——通道方差大于配置差异，
> 跨轮对比以同轮为准。数值一致性校验已移植（`postprocess.check_consistency`，与手撕版同口径），
> 实测检出率 13%，与手撕版 12% 同量级，检出集中在含量测定类数值密集查询——校验器工作正常。
