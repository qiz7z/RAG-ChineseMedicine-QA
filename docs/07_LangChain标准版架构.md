...
>
> **已知优化点**（按预期收益排序）：
> 1. BM25 分词注入药品名/领域词典（手撕版 JiebaTokenizer 有 freq=10000 的领域词，标准版尚未做）；
> 2. 检索评估显示重排为负收益，但生成场景下重排可能改善上下文构成（Q002 类"概要条目挤占临床条目"），需单独做生成侧消融验证；
> 3. 横向/否定性问题的关键词覆盖率受指标字面匹配影响，可考虑语义匹配评估。
